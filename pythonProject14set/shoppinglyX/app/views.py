...
def laptop(request, data=None):
    if data == None:
        ...
        laptops = Product.objects.filter(category='L').filter(discounted_price__lt=16000)
    elif data == 'above':
...
def top_wear(request, data=None):
    ...
    #     top_wears = Product.objects.filter(category='TW').filter(brand=data)
        ...
        top_wears = Product.objects.filter(category='TW').filter(discounted_price__lt=1500)
...
def bottom_wear(request, data=None):
    ...
    #     top_wears = Product.objects.filter(category='TW').filter(brand=data)
        ...
        bottom = Product.objects.filter(category='BW').filter(discounted_price__lt=1500)
...
class CustomerRegistrationView(View):
      ...
      def post(self, request):
           ...
           if form.is_valid():
               messages.success(request, 'Contragulations!! Registered Succussfully')
               form.save()
